...
fn main() {
    ...
    let title = RenderNode::new(
        Rect::new(1, 1, 50, 1),
        NodeContent::Text {
            content: format!("{} Rolo Tree Rendering Demo!", glyph("star")),
            color: Color::Cyan,
        },
    );
    ...
    // Add a real boxy box demo with stateful object!
    ...
    // Bottom left terminal window - DYNAMIC!
    let terminal_box_object = BoxyObject::new(boxy::BoxyConfig {
        text: "$ rolo --version\nrolo 0.1.0 - Tree Rendering Engine".to_string(),
    ...
    // Small accent box
    let accent_box = RenderNode::new(
        Rect::new(45, 15, 15, 3),
        NodeContent::BoxyContent {
            boxy_object: BoxyObject::new(boxy::BoxyConfig {
                text: format!("BREAKTHROUGH! {}", glyph("target")),
                colors: boxy::BoxColors {
    ...
    engine.add_root_child(accent_box);
    ...
    // ANIMATED RENDER TREE! Boxes move every 7 seconds
    println!("\n{} Watch boxes move randomly every 7 seconds! Press Ctrl+C to exit...", glyph("star"));
    ...
    loop {
        ...
        // Move a random box every 7 seconds
        if now.duration_since(last_move) >= Duration::from_secs(7) {
            println!("\n{} Moving random box...", glyph("bolt"));
            engine.move_random_child(terminal_size);
            last_move = now;
        }

        // Update system status every 2 seconds for some life
        if now.duration_since(last_update) >= Duration::from_secs(2) {
            update_counter += 1;

            match update_counter % 4 {
                0 => {
                    status_box_object.update_text("Status: ONLINE\nCPU: 42%\nRAM: 1.2GB".to_string());
                    status_box_object.update_box_color("green".to_string());
                    status_box_object.update_title(Some("System".to_string()));
                }
                1 => {
