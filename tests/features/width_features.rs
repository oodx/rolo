...
fn test_width_module_basic_functionality() {
    ...
    // Test terminal width
    let term_width = rolo::width::get_terminal_width();
    ...
    let width_result = rolo::width::get_display_width(text);
...
fn test_width_validation() {
    ...
    // Valid widths
    assert!(rolo::width::validate_width("80").is_ok());
    assert!(rolo::width::validate_width("120").is_ok());
    assert_eq!(rolo::width::validate_width("80").unwrap(), 80);

    // Invalid widths
    assert!(rolo::width::validate_width("5").is_err());   // too small
    assert!(rolo::width::validate_width("300").is_err()); // too large
    assert!(rolo::width::validate_width("not_a_number").is_err());
    assert!(rolo::width::validate_width("").is_err());
...
fn test_empty_and_edge_cases() {
    ...
    // Empty string
    let empty_width = rolo::width::get_display_width("").unwrap();
    assert_eq!(empty_width, 0, "Empty string should have width 0");

    // Single character
    let single_width = rolo::width::get_display_width("a").unwrap();
...
fn test_width_boxy_features() {
    ...
    let width = rolo::width::get_display_width(ansi_text).unwrap();
    ...
    let unicode_width = rolo::width::get_display_width(unicode_text).unwrap();
...
fn test_width_fallback_behavior() {
    ...
    let width = rolo::width::get_display_width(text).unwrap();
